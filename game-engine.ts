...
import { ResourceChange, ActionType, CompanionTaskType, GameEvent, DecisionEvent } from './types.js';
...
class GameEngine {
  ...
  initializeGame(): void {
    // 初始化同工
    const barnabas = new Companion('barnabas', 'Barnabas', '巴拿巴', 'counselor', '劝慰者', '提升探访效率和士气恢复');
    const silas = new Companion('silas', 'Silas', '西拉', 'resilient', '坚韧者', '降低逼迫带来的体力损耗');
    const timothy = new Companion('timothy', 'Timothy', '提摩太', 'scribe', '忠心代笔者', '协助撰写书信，体力消耗减半');
    const luke = new Companion('luke', 'Luke', '路加', 'healing', '医治', '擅长医治病人');
    const priscilla = new Companion('priscilla', 'Priscilla', '百基拉', 'crafting', '织造', '擅长织造帐篷');
  ...
  private recruitCompanionForCity(): void {
    ...
    if (this.currentCity.name === 'Philippi' && !this.companions.some(c => c.id === 'silas')) {
      ...
      if (this.addCompanion(silas)) {
        ...
        console.log(`   特性：${silas.specialtyName} - ${silas.specialtyDescription}`);
      }
    }
  ...
  private generateActionResult(action: typeof ACTIONS['preach'], companionResults: string[]): string {
    let result = `\n✅ 执行「${action.name}」`;
  ...
  writeLetter(): { success: boolean; message: string } {
    ...
    if (!this.letterSystem.canWriteLetter(this.currentCity.id, this.player)) {
      return { success: false, message: '条件不满足（需要至少3名门徒和30信心值）' };
    }
  ...
  checkGameEnd(): void {
    ...
    if (this.currentTurn >= this.maxTurns) {
      ...
      this.addToLog(this.isVictory ? '时间到，但你完成了主要目标！' : '时间到，未能完成主要目标...');
    }
  }
  ...
  assignCompanionTask(companionId: string, task: CompanionTaskType): { success: boolean; message: string } {
    ...
    if (result.success) {
      ...
      this.player.applyEffects(result.effect);
      this.addToLog(`${companion.nameChinese}执行${taskInfo.nameChinese}任务: ${taskInfo.description}`);
  ...
  private executeEvent(event: GameEvent): { event: GameEvent | DecisionEvent | null; message: string } {
    ...
    if (event.effect.stability) changes.push(`教会健康 ${event.effect.stability > 0 ? '+' : ''}${event.effect.stability}`);
    if (event.effect.persecution) changes.push(`逼迫指数 ${event.effect.persecution > 0 ? '+' : ''}${event.effect.persecution}`);
    if (event.effect.stamina) changes.push(`体力 ${event.effect.stamina > 0 ? '+' : ''}${event.effect.stamina}`);
    if (event.effect.provision) changes.push(`物资 ${event.effect.provision > 0 ? '+' : ''}${event.effect.provision}`);
    if (event.effect.reputation) changes.push(`名声 ${event.effect.reputation > 0 ? '+' : ''}${event.effect.reputation}`);
    if (event.effect.morale) changes.push(`同工士气 ${event.effect.morale > 0 ? '+' : ''}${event.effect.morale}`);
  ...
  handleDecision(eventId: string, choiceIndex: number): string {
    ...
    const changes: string[] = [];
    if (choice.effect.stability) changes.push(`教会健康 ${choice.effect.stability > 0 ? '+' : ''}${choice.effect.stability}`);
    if (choice.effect.persecution) changes.push(`逼迫指数 ${choice.effect.persecution > 0 ? '+' : ''}${choice.effect.persecution}`);
    if (choice.effect.stamina) changes.push(`体力 ${choice.effect.stamina > 0 ? '+' : ''}${choice.effect.stamina}`);
    if (choice.effect.provision) changes.push(`物资 ${choice.effect.provision > 0 ? '+' : ''}${choice.effect.provision}`);
    if (choice.effect.morale) changes.push(`同工士气 ${choice.effect.morale > 0 ? '+' : ''}${choice.effect.morale}`);
  ...
  calculateFinalScore(): {
    ...
    if (finalScore >= 90) {
      ...
      verdict = '【劳苦的福音先锋】';
      description = '"尽管路途艰辛，身体衰弱，但你成功地在关键城市扎下了真理的根。书信成为了你留给后世最宝贵的财富。"';
    } else if (finalScore >= 40) {
      verdict = '【疲惫的守望者】';
      description = '"你到达了罗马，但心中充满了忧虑。部分城市的教会因缺乏教导而动摇。你尽力了，但资源匮乏让你步履维艰。"';
    } else {
      verdict = '【被围困的独行者】';
      description = '"你几乎是孤身一人到达罗马。虽然完成了旅程，但身后的教会网络支离破碎。这趟旅程对你而言是一场惨烈的生还。"';
    }
  ...
  getCompactGameStateDisplay(): string {
    ...
    let state = this.player.getCompactStatus();
  ...
  getCompactAvailableActions(): string {
    ...
    actions.forEach((key, index) => {
      ...
      // 收集正效果
      if (action.effect.stability && action.effect.stability > 0) effects.push(`+${action.effect.stability}健`);
      if (action.effect.provision && action.effect.provision > 0) effects.push(`+${action.effect.provision}物`);
      if (action.effect.stamina && action.effect.stamina > 0) effects.push(`+${action.effect.stamina}体`);
      if (action.effect.faith && action.effect.faith > 0) effects.push(`+${action.effect.faith}信`);
      if (action.effect.reputation && action.effect.reputation > 0) effects.push(`+${action.effect.reputation}声`);
      if (action.effect.disciples && action.effect.disciples > 0) effects.push(`+${action.effect.disciples}徒`);
      
      // 收集负效果
      if (action.effect.stability && action.effect.stability < 0) effects.push(`${action.effect.stability}健`);
      if (action.effect.provision && action.effect.provision < 0) effects.push(`${action.effect.provision}物`);
      if (action.effect.stamina && action.effect.stamina < 0) effects.push(`${action.effect.stamina}体`);
      if (action.effect.persecution && action.effect.persecution > 0) effects.push(`+${action.effect.persecution}逼`);
      
      actionDisplays.push(`[${index + 1}${action.nameChinese.substring(0, 2)}${emojis[index]}${effects.length > 0 ? effects.join('/') : ''}]`);
    });
